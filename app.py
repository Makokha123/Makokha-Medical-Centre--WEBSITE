...
def _send_plain_email(recipient_email, subject, plain_text):
    """Send plain-text email using Resend API; returns (success, message)."""
...
def reply_message():
    ...
    try:
        ...
        if send_email_requested and communication.patient_email:
            ...
            email_sent, email_error = _send_plain_email(
...
def book_appointment():
    ...
    try:
        ...
        appointment_date = datetime.strptime(data.get('appointment_date'), '%Y-%m-%dT%H:%M')
        ...
        if appointment.reception_id:
            notification = Notification(
                ...
                title=f'New appointment booking from {data.get("name")}',
...
def reception_update_appointment(appointment_id):
    ...
    try:
        ...
        if send_email_requested and appointment.patient_email:
            ...
            email_body = (
                ...
                f'Scheduled date/time: {appointment.appointment_date.strftime("%B %d, %Y at %I:%M %p")}.\n'
            ...
            if email_sent:
                flash('Appointment updated and email sent to patient.', 'success')
            else:
                flash(f'Appointment updated. Email not sent: {email_error}', 'warning')
        else:
...
def remove_legacy_seed_reception():
    """Remove the old seeded receptionist account created in earlier versions."""
    legacy_reception = Reception.query.filter_by(
        username='reception',
        email='reception@makokhamedical.com',
        full_name='Reception Team'
    ).first()
    if not legacy_reception:
        return

    Communication.query.filter_by(reception_id=legacy_reception.id).update({'reception_id': None})
    Appointment.query.filter_by(reception_id=legacy_reception.id).update({'reception_id': None})
    Call.query.filter_by(reception_user_id=legacy_reception.id).update({'reception_user_id': None})
    Notification.query.filter_by(reception_id=legacy_reception.id).delete()
    db.session.delete(legacy_reception)
    db.session.commit()
    print("Legacy seeded receptionist account removed.")
